...
/// Menu: Tools / Procedural Cities / Generate LowPoly Models
/// </summary>
public class GenerateLowPolyModels : EditorWindow
{
    ...
    //     return mat;
    // }
    // @todo: END - future remove if isn't really used
    ...
    static int GenerateOven()
    {
        ...
        var matBody   = GetMat("LP_Oven_Body",    new Color(0.88f, 0.88f, 0.86f), 0.15f, 0.4f);
        var matDoor   = GetMat("LP_Oven_Door",    new Color(0.82f, 0.82f, 0.80f), 0.1f, 0.35f);
        var matGlass  = GetMat("LP_Oven_Glass",   new Color(0.12f, 0.10f, 0.08f, 0.5f), 0.05f, 0.9f);
        var matKnob   = GetMat("LP_Oven_Knob",    new Color(0.72f, 0.72f, 0.74f), 0.6f, 0.5f);
        var matBurner = GetMat("LP_Oven_Burner",  new Color(0.08f, 0.08f, 0.08f), 0.5f, 0.3f);
        var matInterior = GetMat("LP_Oven_Interior", new Color(0.18f, 0.18f, 0.20f), 0.3f, 0.2f);
        var matElement = GetMat("LP_Oven_Element", new Color(0.6f, 0.15f, 0.05f), 0.4f, 0.3f);
        var matTray   = GetMat("LP_Oven_Tray",    new Color(0.45f, 0.45f, 0.48f), 0.7f, 0.5f);
        var matHandle = GetMat("LP_Oven_Handle",  new Color(0.65f, 0.65f, 0.68f), 0.7f, 0.55f);
    ...
    static int GenerateKettle()
    {
        ...
        {
            float t0 = (float)i / handleSteps;
            float t1 = (float)(i + 1) / handleSteps;
    ...
    static int GenerateLamp3()
    {
        ...
        // --- Weighted base disc ---
        var basePart = CreateCylinder(0.08f, 0.018f, 8, matBase);
        basePart.transform.SetParent(root.transform);
        basePart.transform.localPosition = new Vector3(0, 0.009f, 0);
        basePart.gameObject.name = "Base";

        // --- Arm: straight vertical pole ---
        float baseTop = 0.018f;
        float armLen = 0.22f;
        var arm = CreateBox(new Vector3(0.016f, armLen, 0.016f), matBase);
        arm.transform.SetParent(root.transform);
        arm.transform.localPosition = new Vector3(0, baseTop + armLen / 2f, 0);
        arm.gameObject.name = "Arm";

        // Arm tip position (straight up)
        float tipX = 0f;
        float tipY = baseTop + armLen;

        // --- Shade (cylinder drum shade, flat top for clean arm connection) ---
        float shadeR = 0.06f;
        float shadeH = 0.05f;
        var head = CreateCylinder(shadeR, shadeH, 8, matShade);
        head.transform.SetParent(root.transform);
        // Raise shade so arm penetrates well into it (no visible gap)
        head.transform.localPosition = new Vector3(tipX, tipY - shadeH / 2f + 0.02f, 0);
        head.gameObject.name = "Head";

        // --- Bulb peeking from shade bottom ---
        var matBulb = GetMat("LP_Lamp_Bulb", new Color(1f, 0.95f, 0.8f), 0.1f, 0.2f);
        var bulb = CreateCylinder(0.018f, 0.015f, 6, matBulb);
        bulb.transform.SetParent(root.transform);
        float shadeBottom = tipY - shadeH / 2f + 0.02f - shadeH / 2f;
        bulb.transform.localPosition = new Vector3(tipX, shadeBottom - 0.005f, 0);
    ...
    static int GenerateTree()
    {
        ...
        // Trunk
        var trunk = CreateCylinder(0.18f, 1.5f, 6, matTrunk);
        ...
        float[] coneR = { 1.35f, 1.05f, 0.75f };
        float[] coneH = { 1.50f, 1.35f, 1.20f };
        float[] coneY = { 1.35f, 2.25f, 3.15f };
        for (int i = 0; i < 3; i++)
        {
    ...
    static int GenerateTree1()
    {
        ...
        // Trunk
        var trunk = CreateCylinder(0.21f, 2.4f, 6, matTrunk);
        trunk.transform.SetParent(root.transform);
        trunk.transform.localPosition = new Vector3(0, 1.2f, 0);
        trunk.gameObject.name = "Trunk";

        // Main canopy sphere
        var canopy = ShapeGenerator.GenerateIcosahedron(PivotLocation.Center, 1.65f, 1);
        canopy.GetComponent<MeshRenderer>().sharedMaterial = matLeaf;
        canopy.transform.SetParent(root.transform);
        canopy.transform.localPosition = new Vector3(0, 3.45f, 0);
        canopy.gameObject.name = "Canopy";

        // Secondary smaller sphere for volume
        var canopy2 = ShapeGenerator.GenerateIcosahedron(PivotLocation.Center, 1.14f, 1);
        canopy2.GetComponent<MeshRenderer>().sharedMaterial = matLeaf;
        canopy2.transform.SetParent(root.transform);
        canopy2.transform.localPosition = new Vector3(0.6f, 3.0f, 0.45f);
    ...
    static int GenerateTree2()
    {
        ...
        var trunk = CreateCylinder(0.27f, 1.8f, 6, matTrunk);
        trunk.transform.SetParent(root.transform);
        ...
        Vector3[] offsets = {
            new Vector3(0, 2.85f, 0),
            new Vector3(0.75f, 2.55f, 0.6f),
            new Vector3(-0.66f, 2.64f, -0.45f),
            new Vector3(-0.3f, 2.40f, 0.84f),
        };
        float[] sizes = { 1.50f, 1.20f, 1.14f, 1.05f };
        for (int i = 0; i < 4; i++)
        {
            var sphere = ShapeGenerator.GenerateIcosahedron(PivotLocation.Center, sizes[i], 1);
            sphere.GetComponent<MeshRenderer>().sharedMaterial = matLeaf;
    ...
    static int GenerateTree3()
    {
        ...
        // Trunk
        var trunk = CreateCylinder(0.195f, 2.1f, 6, matTrunk);
        trunk.transform.SetParent(root.transform);
        trunk.transform.localPosition = new Vector3(0, 1.05f, 0);
        trunk.gameObject.name = "Trunk";

        // Single large cone canopy (autumn yellow-orange)
        var canopy = ShapeGenerator.GenerateCone(PivotLocation.Center, 1.50f, 2.55f, 8);
        canopy.GetComponent<MeshRenderer>().sharedMaterial = TreeLeafYellow;
        canopy.transform.SetParent(root.transform);
        canopy.transform.localPosition = new Vector3(0, 1.95f + 1.275f, 0);
        canopy.gameObject.name = "Canopy";

        // Smaller orange accent cone on top
        var top = ShapeGenerator.GenerateCone(PivotLocation.Center, 0.84f, 1.35f, 7);
        top.GetComponent<MeshRenderer>().sharedMaterial = TreeLeafOrange;
        top.transform.SetParent(root.transform);
        top.transform.localPosition = new Vector3(0, 3.45f + 0.675f, 0);
