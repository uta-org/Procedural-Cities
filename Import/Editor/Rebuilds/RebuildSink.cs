...
public static class RebuildSink
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Sink")]
    static void Rebuild()
    {
        ...
        var oldMatGuids = AssetDatabase.FindAssets("LP_Sink_", new[] { "Assets/LowPoly/Materials" });
        foreach (var guid in oldMatGuids)
        {
            var matPath = AssetDatabase.GUIDToAssetPath(guid);
            if (matPath.Contains("LP_Sink_"))
            {
                AssetDatabase.DeleteAsset(matPath);
                Debug.Log($"[RebuildSink] Deleted old material: {matPath}");
            }
        }
        // Also delete legacy material names
        foreach (var legacyName in new[] { "LP_Ceramic_White", "LP_Ceramic_Inner", "LP_Chrome" })
        {
            var legacyGuids = AssetDatabase.FindAssets(legacyName, new[] { "Assets/LowPoly/Materials" });
            foreach (var guid in legacyGuids)
            {
                var matPath = AssetDatabase.GUIDToAssetPath(guid);
                AssetDatabase.DeleteAsset(matPath);
                Debug.Log($"[RebuildSink] Deleted legacy material: {matPath}");
            }
        }

        var method = genType.GetMethod("GenerateSink",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildSink] GenerateSink method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Sink.prefab";
        string combinedPath = $"{CombinedDir}/Sink_Combined.asset";
        string prefabPath = $"{PrefabDir}/Sink.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildSink] LowPoly_Sink.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildSink] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildSink] Copied material {srcMat.name} to package");
                }
            }
            if (pkgMat != null)
            {
                ...
                if (lpShader != null)
                {
                    ...
                    Debug.Log($"[RebuildSink] Updated material {pkgMat.name}");
                }
            }
        ...
        combinedMesh.name = "Sink_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Sink");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Sink", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Sink"))
            {
        ...
        Debug.Log($"[RebuildSink] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
