...
public static class RebuildClock
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Clock")]
    static void Rebuild()
    {
        ...
        var method = genType.GetMethod("GenerateClock",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildClock] GenerateClock method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Clock.prefab";
        string combinedPath = $"{CombinedDir}/Clock_Combined.asset";
        string prefabPath = $"{PrefabDir}/Clock.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildClock] LowPoly_Clock.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildClock] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                // Copy material to package if it doesn't exist
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildClock] Copied material {srcMat.name} to package");
                }
            }
        ...
        combinedMesh.name = "Clock_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Clock");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Clock", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Clock"))
            {
        ...
        Debug.Log($"[RebuildClock] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
