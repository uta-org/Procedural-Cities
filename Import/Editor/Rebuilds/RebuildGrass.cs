...
public static class RebuildGrass
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Grass")]
    static void Rebuild()
    {
        ...
        var method = genType.GetMethod("GenerateGrass",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildGrass] GenerateGrass method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Grass.prefab";
        string combinedPath = $"{CombinedDir}/Grass_Combined.asset";
        string prefabPath = $"{PrefabDir}/Grass.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildGrass] LowPoly_Grass.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildGrass] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildGrass] Copied material {srcMat.name} to package");
                }
            }
            ...
            if (pkgMat != null)
            {
                ...
                if (lpShader != null && pkgMat.shader != lpShader)
                {
                    ...
                    Debug.Log($"[RebuildGrass] Fixed shader on {pkgMat.name} to LowPolyPBR");
                }
            }
        ...
        combinedMesh.name = "Grass_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Grass");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Grass", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Grass"))
            {
        ...
        Debug.Log($"[RebuildGrass] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
