...
public static class RebuildLamp3
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Lamp3")]
    static void Rebuild()
    {
        Debug.Log("[RebuildLamp3] Step 1: Regenerating LowPoly_Lamp3...");
        ...
        // Delete old Lamp3 materials so GetMat recreates them
        foreach (var matName in new[] { "LP_Lamp_Bulb" })
        {
            var guids = AssetDatabase.FindAssets(matName, new[] { "Assets/LowPoly/Materials" });
            foreach (var guid in guids)
            {
                var matPath = AssetDatabase.GUIDToAssetPath(guid);
                AssetDatabase.DeleteAsset(matPath);
                Debug.Log($"[RebuildLamp3] Deleted old material: {matPath}");
            }
        }

        var method = genType.GetMethod("GenerateLamp3",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildLamp3] GenerateLamp3 method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Lamp3.prefab";
        string combinedPath = $"{CombinedDir}/Lamp3_Combined.asset";
        string prefabPath = $"{PrefabDir}/Lamp3.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildLamp3] LowPoly_Lamp3.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildLamp3] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildLamp3] Copied material {srcMat.name} to package");
                }
            }
            if (pkgMat != null)
            {
                ...
                if (lpShader != null)
                {
                    ...
                    Debug.Log($"[RebuildLamp3] Updated material {pkgMat.name}");
                }
            }
        ...
        combinedMesh.name = "Lamp3_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Lamp3");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Lamp3", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Lamp3"))
            {
        ...
        Debug.Log($"[RebuildLamp3] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
