...
public static class RebuildPan0
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Pan0")]
    static void Rebuild()
    {
        ...
        var oldMatGuids = AssetDatabase.FindAssets("LP_Pan0_", new[] { "Assets/LowPoly/Materials" });
        foreach (var guid in oldMatGuids)
        {
            var matPath = AssetDatabase.GUIDToAssetPath(guid);
            if (matPath.Contains("LP_Pan0_"))
            {
                AssetDatabase.DeleteAsset(matPath);
                Debug.Log($"[RebuildPan0] Deleted old material: {matPath}");
            }
        }

        var method = genType.GetMethod("GeneratePan0",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildPan0] GeneratePan0 method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Pan0.prefab";
        string combinedPath = $"{CombinedDir}/Pan0_Combined.asset";
        string prefabPath = $"{PrefabDir}/Pan0.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildPan0] LowPoly_Pan0.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildPan0] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildPan0] Copied material {srcMat.name} to package");
                }
            }
            if (pkgMat != null)
            {
                ...
                if (lpShader != null)
                {
                    ...
                    Debug.Log($"[RebuildPan0] Updated material {pkgMat.name}");
                }
            }
        ...
        combinedMesh.name = "Pan0_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Pan0");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Pan0", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Pan0"))
            {
        ...
        Debug.Log($"[RebuildPan0] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
