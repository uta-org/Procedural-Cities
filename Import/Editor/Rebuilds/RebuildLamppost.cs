...
public static class RebuildLamppost
{
    ...
    // [MenuItem("Procedural Cities/Rebuild Lamppost")]
    static void Rebuild()
    {
        ...
        var method = genType.GetMethod("GenerateLamppost",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildLamppost] GenerateLamppost method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Lamppost.prefab";
        string combinedPath = $"{CombinedDir}/Lamppost_Combined.asset";
        string prefabPath = $"{PrefabDir}/Lamppost.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildLamppost] LowPoly_Lamppost.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildLamppost] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildLamppost] Copied material {srcMat.name} to package");
                }
            }
            if (pkgMat != null)
            {
                ...
                if (lpShader != null && pkgMat.shader != lpShader)
                {
                    ...
                    Debug.Log($"[RebuildLamppost] Fixed shader on {pkgMat.name} to LowPolyPBR");
                }
            }
        ...
        combinedMesh.name = "Lamppost_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Lamppost");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Lamppost", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Lamppost"))
            {
        ...
        Debug.Log($"[RebuildLamppost] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
