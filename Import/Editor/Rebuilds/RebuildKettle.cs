...
public static class RebuildKettle
{
    ...
    // [MenuItem("Tools/uzProceduralCities/Rebuild Kettle")]
    static void Rebuild()
    {
        ...
        var method = genType.GetMethod("GenerateKettle",
            System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.NonPublic);
        if (method == null)
        {
            Debug.LogError("[RebuildKettle] GenerateKettle method not found!");
            return;
        }
        method.Invoke(null, null);

        string lowPolyPath = $"{LowPolyDir}/LowPoly_Kettle.prefab";
        string combinedPath = $"{CombinedDir}/Kettle_Combined.asset";
        string prefabPath = $"{PrefabDir}/Kettle.prefab";

        var lowPoly = AssetDatabase.LoadAssetAtPath<GameObject>(lowPolyPath);
        if (lowPoly == null)
        {
            Debug.LogError("[RebuildKettle] LowPoly_Kettle.prefab not found at " + lowPolyPath);
            return;
        }
        ...
        Debug.Log($"[RebuildKettle] Collected {parts.Count} mesh parts");
        ...
        foreach (var kvp in materialGroups)
        {
            ...
            if (pkgMat == null)
            {
                ...
                if (!string.IsNullOrEmpty(srcPath))
                {
                    ...
                    Debug.Log($"[RebuildKettle] Copied material {srcMat.name} to package");
                }
            }
            if (pkgMat != null)
            {
                ...
                if (lpShader != null && pkgMat.shader != lpShader)
                {
                    ...
                    Debug.Log($"[RebuildKettle] Fixed shader on {pkgMat.name} to LowPolyPBR");
                }
            }
        ...
        combinedMesh.name = "Kettle_Combined";
        ...
        using (var scope = new PrefabUtility.EditPrefabContentsScope(prefabPath))
        {
            ...
            var label = root.transform.Find("Label_Kettle");
            if (label != null)
        ...
        var staleGuids = AssetDatabase.FindAssets("LowPoly_Kettle", new[] { LowPolyDir });
        int deleted = 0;
        foreach (var guid in staleGuids)
        {
            var p = AssetDatabase.GUIDToAssetPath(guid);
            if (p.Contains("LowPoly_Kettle"))
            {
        ...
        Debug.Log($"[RebuildKettle] Done! Bounds: {combinedMesh.bounds.size}, verts={combinedMesh.vertexCount}, mats={finalMaterials.Count}, cleaned {deleted} intermediate assets");
    }
